...
io.on("connection", (socket) => {
  ...
  socket.on("joinAssetRoom", (asset) => {
    ...
    if (roomSize === 1) {
      ...
      roomCount++;
      console.log(
        `New room created for asset: ${asset}. Total rooms: ${roomCount}`
      );
    }
  ...
  socket.on("leaveAssetRoom", (asset) => {
    ...
    if (roomSize === 0) {
      ...
      console.log(
        `Room for asset ${asset} is empty. Total rooms: ${roomCount}`
      );
    }
  });
  ...
  socket.on("disconnect", () => {
    ...
    rooms.forEach((room) => {
      ...
      if (roomSize === 0) {
        ...
        roomCount--; // Decrement the room count
        console.log(
          `Room for asset ${room} is empty. Total rooms: ${roomCount}`
        );
      }
    });
  });
...
// function fetchTokenDeriv() {
//   const ws = new WebSocket(DERIV_API_URL);

//   ws.on("open", () => {
//     const request = {
//       authorize: "a1-mRwF3BTkYT1A6nuKY2Pp2y51GQpWQ",
//     };
//     ws.send(JSON.stringify(request));
//   });
